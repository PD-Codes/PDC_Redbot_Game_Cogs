...
log = logging.getLogger("red.karlo-cogs.warcraftlogs-retail.http")
...
async def generate_bearer(bot: Red, config: Config) -> Optional[str]:
    ...
    NOTE (retail cog): client_id/client_secret are shared with the classic cog
    via ``get_shared_api_tokens("warcraftlogs")``, but the bearer is stored in
    this cog's OWN Config (``config.bearer`` / ``config.bearer_timestamp``) so it
    does not clobber the classic cog's bearer (different domain)."""
    ...
    if not client_id:
        ...
        async with aiohttp.ClientSession(
            ...
            )  # Round the timestamp down to a full number
            bearer_timestamp = (
            ...
            # Store the retail bearer in this cog's own Config (NOT shared tokens)
            # so it never overwrites the classic cog's bearer.
            await config.bearer.set(json["access_token"])
...
class WoWLogsClient:
    ...
    async def get_zones(self):
        """Fetch the list of WCL zones (raids) live from the API.

        Retail zone IDs change every patch, so they must not be hardcoded.
        Returns the list under ``data.worldData.zones`` or ``None`` on error."""
        async with self.session.post(
            graphql_url, json={"query": Queries.get_zones}
        ) as call:
            try:
            ...
            try:
                return json["data"]["worldData"]["zones"]
